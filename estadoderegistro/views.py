...
from estadoderegistro.models import Estado, TipoDeRegistro, \
    EstadoDeRegistro
from estadoderegistro.forms import EstadoForm, TipoDeRegistroForm, \
    EstadoDeRegistroForm
...
# app tipo de registro
class TipoDeRegistroListView(ListView):
    model = TipoDeRegistro
    paginate_by = 10
    context_object_name = 'tiposderegistro'
    template_name = 'tipoderegistro_lista.html'
    ...
    def get_context_data(self, **kwargs):
        # Call the base implementation first to get a context
        context = super(TipoDeRegistroListView, self).get_context_data(**kwargs)
        # Add in the pais
        # if self.request.user.id is not None:
        #     range_gap = valor_Personalizacionvisual(self.request.user.id, "rangopaginacion")
        # else:
        ...
        order_by = self.request.GET.get('order_by')
        if order_by:
            lista_tipoderegistro = TipoDeRegistro.objects.all().order_by(order_by)
        else:
            lista_tipoderegistro = TipoDeRegistro.objects.all()

        paginator = Paginator(lista_tipoderegistro, 10)
        page = self.request.GET.get('page')
        if page:
    ...
    def get_queryset(self):
        ...
        order_by = self.request.GET.get('order_by')
        if order_by:
            queryset = TipoDeRegistro.objects.all().order_by(order_by)
        else:
...
class TipoDeRegistroView(View):
    form_class = TipoDeRegistroForm
    template_name = 'tipoderegistro_add.html'
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            if 'regEdit' in request.POST:
                messages.success(request, "Registro guardado.")
                return HttpResponseRedirect(reverse('uestadoderegistros:edit_tipoderegistro',
                                                    args=(id_reg.id,)))
            else:
                return HttpResponseRedirect(reverse('uestadoderegistros:list_tipoderegistro'))

        return render(request, self.template_name, {'form': form})


class TipoDeRegistroUpdate(UpdateView):
    template_name = 'tipoderegistro_edit.html'
    form_class = TipoDeRegistroForm
    model = TipoDeRegistro

    def get_context_data(self, **kwargs):
        # Call the base implementation first to get a context
        context = super(TipoDeRegistroUpdate, self).get_context_data(**kwargs)
        # if self.request.user.id is not None:
        #     nropag = valor_Personalizacionvisual(self.request.user.id, "paginacion")
        # else:
        #     nropag = valor_Personalizacionvisual("std", "paginacion")
        ...
        tipoderegistro = TipoDeRegistro.objects.get(pk=self.object.pk)
        redirect_to = self.request.REQUEST.get('next', '')
        order_by = self.request.REQUEST.get('order_by', '')
        page = self.request.REQUEST.get('page', '')
        ...
        paginator = Paginator(lista_tipoderegistro, nropag)
        # Show 25 contacts per page

        if page == '0':
            tiposdeRegistro = lista_tipoderegistro
        else:
            try:
                tiposdeRegistro = paginator.page(page)
            except PageNotAnInteger:
                # If page is not an integer, deliver first page.
                tiposdeRegistro = paginator.page(1)
            except EmptyPage:
                # If page is out of range (e.g. 9999), deliver last page of results.
                tiposdeRegistro = paginator.page(paginator.num_pages)

        if page != '0':
            countitem = int(nropag)
            for i in range(0, countitem):
                if(tiposdeRegistro.object_list[i].id == tipoderegistro.id):
                    if tiposdeRegistro.has_previous:
                        try:
                            previousitem = tiposdeRegistro.object_list[i-1].id
                        except:
                            previousitem = None

                    if tiposdeRegistro.has_next:
                        try:
                            nextitem = tiposdeRegistro.object_list[i+1].id
                        except:
                            nextitem = None
                    break
        else:
            countitem = len(tiposdeRegistro)
            for i in range(0, countitem):
                if(tiposdeRegistro[i].id == tipoderegistro.id):
                    try:
                        previousitem = tiposdeRegistro[i-1].id
                    except:
                        previousitem = None
                    try:
                        nextitem = tiposdeRegistro[i+1].id
                    except:
                        nextitem = None
                    break

        try:
            tipoderegistro_previous = TipoDeRegistro.objects.get(pk=previousitem)
        except:
            tipoderegistro_previous = None
        try:
            tipoderegistro_next = TipoDeRegistro.objects.get(pk=nextitem)
        except:
            tipoderegistro_next = None

        context['tipoderegistro_previous'] = tipoderegistro_previous
        context['tipoderegistro_next'] = tipoderegistro_next
...
class TipoDeRegistroDelete(DeleteView):
    model = TipoDeRegistro
    form_class = TipoDeRegistroForm
    template_name = 'server_confirm_delete.html'
